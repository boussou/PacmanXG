...
uses
 ...
 mseifiglob{,mseapplication},msewidgetgrid,mseimage,
 (*patch by Alexandre Minoshi*)mseformatpngread, mseformatjpgread,
 mseformatxpmread,mseformatbmpicoread,mseformatpnmread,mseformattgaread,
 registry(*End of patch*);
...
//   property dialogkind: filedialogkindty read fdialogkind write fdialogkind default fdk_open;
 ...
 tfiledialogfo = class(tmseform)
   ...
   procedure on_cellevent(const sender: TObject; var info: celleventinfoty);
   procedure on_showsort(const sender: TObject);
   procedure on_sortbyname(const sender: TObject);
   procedure on_sortbytime(const sender: TObject);
   procedure on_sortbytype(const sender: TObject);
   procedure on_sortbysize(const sender: TObject);
  private
 ...
 with tfilelistview(sender) do begin
  if iscellclick(info) then begin
   ...
   if filelist.isdir(index) then begin
    ...
    (*Patch of Alexandre Minoshi*) 
    if tbooleanedit1.value then
    BEGIN
    _fname := filepath(directory+filelist[index].name);
    try
      timage1.bitmap.loadfromfile(_fname);
      timage1.visible := true;
    except
      try
        tmemoedit1.value := '';
        i := 0;
        {$ifdef windows}if _fname[1] = '/' then delete(_fname,1,1);{$endif}
        assignfile(f,_fname);
        reset(f);
        while not eof(f) do
          begin
            readln(f,s);
            tmemoedit1.value := tmemoedit1.value + s + #10;
            tmemoedit1.visible := true;
            inc(i);
            if i = 6 then break;
          end;
        tmemoedit1.value := trim(tmemoedit1.value);
        closefile(f);
      except
      end;
    end; //try1
...
// aligny(wam_center,[dir,home,up,createdir]);
 aligny(wam_center,[dir,up,createdir, SORT]);
...
(*Patch of Alexandre Minoshi*)
procedure tfiledialogfo.on_show(const sender: TObject);
...
begin
  r := tstringgrid1.row;
  tstringgrid1.clear;
  tstringgrid1.appendrow(['Root','/']);
  tstringgrid1.appendrow(['Application dir',sys_getapphomedir]);
  tstringgrid1.appendrow(['Temp',sys_gettempdir]);
  tstringgrid1.appendrow(['Home',sys_getuserhomedir]);
  {$ifdef linux}
  tstringgrid1.appendrow(['Desktop',sys_getuserhomedir + '/Desktop']);
  tstringgrid1.appendrow(['Programs','/usr/share/applications/']);
  {$endif};
  {$ifdef Windows}
  tstringgrid1.appendrow(['Network','//']);
  {$endif};

  {$ifdef linux}
  if not fileexists(sys_getuserhomedir + '/.gtk-bookmarks') then exit;
  sl := tstringlist.create;
  sl.loadfromfile(sys_getuserhomedir + '/.gtk-bookmarks');
  
  if sl.count > 0 then
    for i := 0 to sl.count - 1 do
      begin
        s2 := URLDecode(trim(sl[i]));
        if system.pos('file://',s2) = 1 then delete(s2,1,7);
        
        if directoryexists(s2) then
        BEGIN
        if system.pos(' ',s2) > 0 
        then begin
             s1 := copy(s2,1,system.pos(' ',s2) - 1);
             delete(s2,1,system.pos(' ',s2));
             tstringgrid1.appendrow([s1,s2]);
             end
        else tstringgrid1.appendrow([extractfilename(s2),s2]);
        END;
      end;
  sl.free;
  {$endif}  
  ...
  s := reg.readstring('Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders','Desktop','');
  if s > '' then tstringgrid1.appendrow([extractfilename(s),s]);
  s := reg.readstring('Software\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders','Favorites','');
  reg.free;
  if s > '' then 
  begin
    tstringgrid1.appendrow([extractfilename(s),s]);  
    if findfirst(s + '\*', faanyfile, fs) = 0
    then repeat
         if (fs.name = '.')or(fs.name = '..') or (fs.name = '') then continue;
         if (fs.attr and fadirectory) <> 0 then 
            tstringgrid1.appendrow([fs.name,s + '\' + fs.name]);  
         until findnext(fs) <> 0;
    findclose(fs);
  end;
  {$endif}
  if ( r >= 0 ) and (r <= tstringgrid1.rowhigh) then tstringgrid1.row := r;
end;

procedure tfiledialogfo.on_cellevent(const sender: TObject; var info: celleventinfoty);
begin
  if iscellclick(info, [ccr_buttonpress]) then 
     if tstringgrid1.row >= 0 then tryreadlist(tstringgrid1[1][tstringgrid1.row]);
end;

procedure tfiledialogfo.on_showsort(const sender: TObject);
 var gr : mouseeventinfoty;
begin
  gr.pos.x := 0;
  gr.pos.y := sort.height;
  tpopupmenu1.show(sort, gr);
end;

procedure tfiledialogfo.on_sortbyname(const sender: TObject);
begin 
 listview.filelist.options := [flo_sortname,flo_sorttype];
 listview.readlist;
end;

procedure tfiledialogfo.on_sortbytime(const sender: TObject);
begin
 listview.filelist.options := [flo_sorttime,flo_sorttype];
 listview.readlist;
end;

procedure tfiledialogfo.on_sortbytype(const sender: TObject);
begin
 listview.filelist.options := [flo_sorttype];
 listview.readlist;
end;

procedure tfiledialogfo.on_sortbysize(const sender: TObject);
begin
 listview.filelist.options := [flo_sortsize,flo_sorttype];
 listview.readlist;
end;
